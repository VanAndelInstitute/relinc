...
Make sure the redis-server is configured to store the snapshot somewhere 
with enough room (the default location is fine if your root drive had 50GB or
so free), and to allow connections from outside (by commenting out the "bind" 
line in the network section of redis.conf) if you are going to use 
separate servers to do the analysis (see below).  Remember, "outside" means 
"other servers in your security group or behind your firewall", not the 
entire world.
...
This analysis requires the latest versions of `relinc` and `slinky`.
